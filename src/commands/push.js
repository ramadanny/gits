  ...
  program
    ...
    .action(async (targetPath, message, options) => {
      try {
        ...
        if (detectedSecrets.length > 0) {
          ...
          for (const secret of detectedSecrets) {
            if (!gitignoreContent.includes(secret)) {
              isIgnored = false;
              global.log.error(`CRITICAL: Sensitive file "${secret}" detected and NOT ignored!`);
            }
          }
          if (!isIgnored) {
        ...
        if (remotes.length === 0) {
          global.log.error('Remote "origin" not found. Add it using "gits remote add <url>"');
          return;
        }

        const status = await git.status();
        const branch = status.current || 'main';
        const targetRemotes = options.all ? remotes : [remotes.find(r => r.name === 'origin')].filter(Boolean);

        for (const remote of targetRemotes) {
            const remoteUrl = remote.refs.push.replace('https://', `https://${username}:${token}@`);
